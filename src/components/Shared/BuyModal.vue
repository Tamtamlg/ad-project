<template>
  <v-dialog width="400px" v-model="modal">
    <v-btn class="primary mr-2" slot="activator">Buy</v-btn>
    <v-card>
      <v-container>
        <v-layout row>
          <v-flex xs12>
        ...
        <v-layout row>
          <v-flex xs12>
            <v-card-text>
              ...
              <v-text-field name="phone" label="Your phone" type="tel" v-model="phone"></v-text-field>
            </v-card-text>
          </v-flex>
        </v-layout>
        <v-layout row>
          <v-flex xs12>
            <v-card-actions>
              <v-spacer></v-spacer>
              <v-btn class="default" @click="onCancel" :disabled="localLoading">Cancel</v-btn>
